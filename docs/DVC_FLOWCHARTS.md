...
3. **DPAリバランス** … 売りは「総資産に対する現在比率」と「**保有銘柄のみで正規化した**ターゲット構成比 `w_i^*`」の差分で判定。買いは「**ポートフォリオ用 total_score の降順**」で候補を並べたうえで、**仮想組入（Simulated Inclusion）と動的N最適化（Dynamic N-Optimization）**で N=1〜5 のシナリオをシミュレーションし、予算消化・分散・スコアのバランスが最も良いシナリオを自動選択してテキストレポートを出力。
...
- **売り（パージ）**: 保有銘柄ごとに、**総資産に対する現在比率** `w` と、同じく総資産ベースの**保有のみで山分けしたターゲット** `w_i^*`（`compute_target_weights(..., allocation_tickers=保有ティッカー)`）を比較する。`over = max(0, w - w_i^*)` が `over_weight_threshold`（既定 2%pt）を超えた銘柄を売却候補にする（`run_purge`）。
- **買い**: 購入候補は**ポートフォリオ用 total_score の降順**で並べ（ウォッチの WATCHING かつ着火点を満たす銘柄。**保有済みも候補**）、予算・1銘柄上限（15%・75万円固定）・100株単位で割り当てる（後述）。
