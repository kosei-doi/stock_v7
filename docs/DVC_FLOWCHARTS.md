...
3. **DPAリバランス** … 売りは「保有銘柄だけのターゲット比率」との差分で判定。買いは「**ポートフォリオ用 total_score の降順**」で候補を並べたうえで、**仮想組入（Simulated Inclusion）と動的N最適化（Dynamic N-Optimization）**で N=1〜5 のシナリオをシミュレーションし、予算消化・分散・スコアのバランスが最も良いシナリオを自動選択してテキストレポートを出力。
...
- **売り（パージ）用のターゲット**: 上記で求めた `w_i^*` のうち**保有銘柄だけ**を抜き出し、非現金部分で再正規化したものを「保有銘柄どうしの目標比率」として使う。`w_i > w_i^* + しきい値` の銘柄を売却候補にする。
...
- **売却**: ターゲットは「保有銘柄だけで山分け」した比率。現在比率がそれを上回る銘柄を縮小（利確・損切りはこの差分で判断）。
...
*このドキュメントは DVC Phase 1 と DPA の最新実装（β/R²/α/ATR 活用・保有銘柄のみ山分け・予算から埋める購入・レポート仕様）に基づいています。*
